...
public class CommandParser {
  ...
  public static Namespace parseArgs(String[] args) {
    ArgumentParser parser =
        ArgumentParsers.newFor("Stack Exchange CLI")
            .build()
            .defaultHelp(true)
            .description("Search stackexchange websites");

    parser
        .addArgument("-q", "--query")
        .required(true)
        .help("Search query used to " + "search a stack exchange website");

    parser
        .addArgument("-s", "--site")
        .setDefault("stackoverflow")
        .help("Stack " + "exchange website used to search the query on - default=stackoverflow");

    parser
        .addArgument("-t", "--tags")
        .help(
            "Space seperated tags used in stack "
                + "exchange search -"
                + " "
